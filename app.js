...
const server = http.createServer((request, response) => {
  ...
  if (request.url === "/health") {
    ...
    response.end(JSON.stringify({ status: "UP", service: "cloud-orders", version, environment }));
    return;
  }
  ...
  if (request.url === "/") {
    response.writeHead(200);
    response.end(JSON.stringify({ message: "Welcome to Cloud Orders", endpoints: ["/health", "/api/orders"], version }));
    return;
  }
